...
Оновлено для версії моду ***1.15.12***<!--, патчу від *25&nbsp;січня 2025*--> (версія гри&nbsp;— *1.17*)
...
+ [x] `ww1_diplomacy`
+ [ ] ~~`ww1_doctrines`~~
+ [x] `ww1_equip_air` (майже)
+ [ ] `ww1_equipment` — (збер. ст. пер.)
...
+ [x] `replace_diplomacy`
+ [x] `replace_effects`
+ [ ] `replace_events` — (збер. ст. пер.)
+ [ ] ~~`replace_focus`~~
+ [x] `replace_frontend`
+ [ ] `replace_game_rules` — (збер. ст. пер.)
+ [ ] `replace_ideas`
+ [x] `replace_military_industrial_organization`
+ [x] `replace_modifiers`
+ [x] `replace_mtg_decisions`
+ [x] `replace_music`
+ [x] `replace_operatives` (умовно, аааааа)
+ [x] `replace_peace` (бле)
+ [x] `replace_politics`
+ [ ] `replace_research` — (збер. ст. пер.)
+ [ ] ~~`replace_special_projects`~~
+ [x] `replace_state`
+ [x] `replace_stats` (аааааа)
+ [x] `replace_tank_modules`
+ [x] `replace_traits`
+ [x] `replace_triggers`
+ [x] `replace_unit`
